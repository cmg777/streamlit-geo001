...
INDICATOR2 = 'pop2020'      # For size of sunburst segments
...
# Sidebar Selectors
st.sidebar.header("Sunburst Configuration")
...
# --- Create Sunburst ---
st.subheader(f"Sunburst of Municipalities by Department")
...
fig = px.sunburst(
    ...
    values=selected_size_col,              # Data for size of sunburst segments (e.g., population)
...
This sunburst diagram organizes municipalities hierarchically by department in a radial layout. Each segment represents:

- **Size** proportional to the selected size indicator
- **Color** intensity based on the selected color indicator
- **Hover** showing municipality name and additional selected indicator

Click on segments to zoom in and explore departments in detail. Click in the center to zoom out.
""")